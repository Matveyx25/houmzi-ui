...
export const getServerSideProps: GetServerSideProps = wrapper.getServerSideProps(
  async (cxt: GetServerSidePropsContext<{ slug: string }> & { store: Store }) => {
    const slug = cxt.params.slug
    if (slug.length <= 35) return { redirect: { permanent: false, destination: '/blog' } };
    const articleId = slug.slice(slug.length - 36, slug.length)    
